...
export async function fetchGitHubRepos(username, token = null, organizations = []) {
  ...
  try {
    ...
    // Use authenticated endpoint for better reliability when token is available
    // The /user/repos endpoint returns all repos including recently created ones
    const ownedRepos = [];
    let repoPage = 1;
    ...
    while (repoPage <= maxRepoPages) {
      ...
      const pageRepos = await ownedResponse.json();

      if (pageRepos.length === 0) {
        break; // No more repos
      }

      ownedRepos.push(...pageRepos);
    ...
    console.log(`Found ${ownedRepos.length} owned repositories (${repoPage} page${repoPage > 1 ? 's' : ''})`);

    // Filter to only include repos owned by the user (not forks from authenticated endpoint)
    const filteredOwnedRepos = ownedRepos.filter(repo =>
      repo.owner?.login?.toLowerCase() === username.toLowerCase()
    );

    if (filteredOwnedRepos.length !== ownedRepos.length) {
      console.log(`Filtered to ${filteredOwnedRepos.length} repos owned by ${username}`);
    }
