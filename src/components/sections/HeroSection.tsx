...
import { useParallaxAnimation, useTextRevealAnimation, useScrollAnimation, useMouseInteraction, useMagneticEffect, useBreathingEffect, useRippleEffect } from '../../hooks'
import { ANIMATION_DURATION, ANIMATION_EASE } from '../../lib'
...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const projectsButtonRef = useMagneticEffect<HTMLButtonElement>({ strength: 0.4, threshold: 80 })
  const contactButtonRef = useMagneticEffect<HTMLButtonElement>({ strength: 0.3, threshold: 60 })

  // Add breathing effect to animation container
  const breathingRef = useBreathingEffect({ intensity: 0.03, duration: 4000 })

  // Add ripple effect to buttons
  const { triggerRipple: triggerProjectsRipple, ripples: projectsRippleElements } = useRippleEffect()
  const { triggerRipple: triggerContactRipple, ripples: contactRippleElements } = useRippleEffect()
        ...
        >
            ...
            >
              ...
              >
                <Button 
                  ...
                  onClick={(e) => triggerProjectsRipple(e)}
                >
                  {ctaProjects}
                  <ArrowRight className="ml-2 h-5 w-5 transition-transform group-hover:translate-x-1" />
                  {projectsRippleElements}
                </Button>
              </motion.div>
              ...
              >
                <Button 
                  ...
                  size="lg" 
                  className="group relative overflow-hidden"
                  onClick={(e) => triggerContactRipple(e)}
                >
                  <Mail className="mr-2 h-5 w-5" />
                  {ctaContact}
                  {contactRippleElements}
                </Button>
              </motion.div>
            </motion.div>
          </div>
          ...
          >
            <div
              ref={(el) => {
                if (el) {
                  // Assign to both refs for different effects
                  animationRef.current = el
                  breathingRef.current = el
                }
              }}
